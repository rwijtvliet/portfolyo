...
def test_flatpfline_asfreqcorrect1(freq_in: str, freq_out: str, columns: str, dtype: str):
    ...
    pfl_in = create.flatpfline({col: series[freq_in][col].astype(dtype) for col in columns})

    expected_out = create.flatpfline({col: series[freq_out][col].astype(dtype) for col in columns})
