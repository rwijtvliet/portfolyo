...
from portfolyo import FlatPfLine, PfState, dev, testing  # noqa
...
@pytest.mark.parametrize(
    ...
    [
        (  # Full package.
            ...
            FlatPfLine({"w": s_ref + 20, "p": s_ref * 20}),
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_ref * 10}),
            FlatPfLine({"w": s_ref + 20, "p": s_ref * 20}),
        ),
        (  # Too much information at offtake.
            FlatPfLine({"w": s_ref, "p": s_ref + 1}),
            FlatPfLine({"p": s_ref * 10}),
            FlatPfLine({"w": s_ref + 20, "p": s_ref * 20}),
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_ref * 10}),
        ...
        (  # Too much information at unsourcedprice.
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_ref * 10, "w": s_ref + 10}),
            FlatPfLine({"w": s_ref + 20, "p": s_ref * 20}),
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_ref * 10}),
            FlatPfLine({"w": s_ref + 20, "p": s_ref * 20}),
        ...
        (  # Unequal periods; result is trimmed.
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_more * 10}),
            FlatPfLine({"w": s_more + 20, "p": s_more * 20}),
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_more * 10}).loc[i_ref],
            FlatPfLine({"w": s_more + 20, "p": s_more * 20}).loc[i_ref],
        ),
        (  # Unequal periods; error (intersection; not enough sourced volume).
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_more * 10}),
            FlatPfLine({"w": s_less + 20, "p": s_less * 20}),
            ValueError,
            None,
            None,
        ),
...
def test_makepflines_initpfstate(o, u, s, o_exp, u_exp, s_exp):
    ...
    if s_exp is None:
        sourced_exp = FlatPfLine(pd.DataFrame({"q": 0, "r": 0}, o_exp.index))
    else:
        sourced_exp = s_exp
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (  # Full package with energy and price.
            ...
            FlatPfLine({"q": s_ref}),
            FlatPfLine({"p": s_ref * 10}),
            FlatPfLine({"q": s_ref + 20, "p": s_ref * 20}),
        ),
        (  # Full package with power and revenue.
            ...
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_ref * 10}),
            FlatPfLine({"w": s_ref + 20, "r": s_ref * 20}),
        ),
        (  # Full package with price and revenue.
            ...
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_ref * 10}),
            FlatPfLine({"p": s_ref + 20, "r": s_ref * 20}),
        ),
        (  # Full package with energy and revenue.
            ...
            FlatPfLine({"q": s_ref}),
            FlatPfLine({"p": s_ref * 10}),
            FlatPfLine({"q": s_ref + 20, "r": s_ref * 20}),
        ),
        ...
        (  # No sourcing yet.
            ...
            FlatPfLine({"w": s_ref}),
            FlatPfLine({"p": s_ref * 10}),
            FlatPfLine(pd.DataFrame({"q": 0.0, "r": 0.0}, i_ref)),
        ),
...
def test_pfstate_consistency_uniformfreq():
    ...
    # Expected.
    o_exp = FlatPfLine({"q": -qo})
    s_exp = FlatPfLine({"q": qs, "r": rs})
    u_exp = FlatPfLine({"p": pu})
    # Test.
    assert result.offtakevolume == o_exp
    assert result.unsourcedprice == u_exp
    assert result.sourced == s_exp
