...
def weights1d(
    ...
    if weights1d_as is dict:
        return {
            i: addunitsfn(num) for i, num in zip(weights1d_rowindex[::-1], weights1d_numbers[::-1])
        }
...
def expected_for_values1d_and_weights0d(
    ...
    return pf.Q_(expected_numbers, "Eur/MWh") if values1d_has_units else expected_numbers
