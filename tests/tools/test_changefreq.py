...
def idfn_0600(longestfreq: str) -> Callable[[pd.DatetimeIndex], Iterable[Tuple[int]]]:
    ...
    def numofdaysinprevmonth(i):
        return month(i).map(lambda m: [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][m - 1]) + (
            i.year == 2020
        ) * (month(i) == 2)
...
def startdate(freq):
    if freq == "15min" or freq == "h" or freq == "D":
        return "2019-12-15"
    if freq == "MS":
        return "2019-12-01"
...
def test_index_new_freq(
    ...
    expected_result = pd.date_range(expected_result[0], expected_result[1], freq=freq[1])
