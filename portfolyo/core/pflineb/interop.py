...
from ... import toolsb
from ...toolsb.types import COLS, PintTimeSeries, TimeDataframe, TimeSeries
...
class InOp:
    ...
    . Turn all fields into timeseries (in-place):
        inop.to_timeseries()

    . Check for consistency and fill the missing fields as much as possible (in-place):
    ...
    def __post_init__(self):
        ...
        # Ensure the dimensionality of all values are compatible with each other.
        toolsb.wqpr.validate_compatible(self.fields.values())
    ...
    def to_timeseries(self, ref_index: pd.DatetimeIndex | None = None) -> None:
        ...
        for field, value in self.fields.items():
            if isinstance(value, pint.Quantity):
                newfields[field] = pd.Series(value.magnitude, index).astype(f"pint[{value.units}]")
            else:  # pinttimeseries
                newfields[field] = value.loc[index]
        self.fields = newfields

    def make_consistent(self) -> None:
        """Fill as much of the data as possible. All data must be None or timeseries."""
        wqpr_fields = {field: self.fields.get(field) for field in COLS}
        w, q, p, r = toolsb.wqpr.complete_and_verify(**wqpr_fields)
        if w is not None:
            self.fields["w"] = w
        if q is not None:
            self.fields["q"] = q
    ...
    def to_df(self) -> pd.DataFrame:
        """Create dataframe with (at most) columns w, q, p, r, nodim."""
...
def _equal(inop1: InOp, inop2: InOp) -> bool:
    """2 ``InOp`` objects are equal if they have the same fields with the same values."""
