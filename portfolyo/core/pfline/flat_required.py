...
class FlatRequiredMethods:
    ...
    def set_commodity(self, commodity: Commodity | None) -> FlatPfLine:
        ...
        if self.commodity is commodity:  # already correct
            return self
        elif commodity is None:  # need to remove commodity
            return flat.FlatPfLine(self.df, self.kind, None)
        else:  # need to change the commodity
            newdf = flat_helper.apply_commodity(self.df, commodity)
