...
def dataframe(
    ...
    if isinstance(weights, pd.DataFrame):
        ...
        if surplusweights := set(weights.columns) - set(df.columns):
            raise ValueError(f"No values found for one or more weights: {surplusweights}.")
...
def weights_as_floatseries(weights: Iterable | Mapping, refindex: Iterable) -> pd.Series:
